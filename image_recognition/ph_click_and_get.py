...
works well on the training data
unfortunately breaks down on different images
need more training sets
"""
import pickle
import cv2
import numpy as np
...
def on_mouse(event):
    ...
    test = np.vstack([red, green, blue]).T
    ph_predict = clf.predict(test)
    text.set("pH: " + str(ph_predict).strip('[]'))


pickle_in = open('v4gradient.pickle', 'rb')
